...
OUT.PATH <- "./RPlots/"
# Create out Folder if not existent yet
...
# Sanitize version, when there are characters that need to be escaped
...
# Relable Methods:
# Takes as input: vector of former labels
# Returns: vector of new labels
...
reduction_name <- "RMD-variant" # "Reduction"
...
# Restrict to "hard" instance
dfh <- subset(df, Difficulty == "hard")
# Filter for machines > 1, that is Terms with at least deg 4 before the reduction
dfh <- dfh %>%
    filter(NoMachines != 1)

###################################################################
####### Introduced Vars / Runtime plot
###################################################################

# Create a joint plot (facet) for the number of variables after / before and the runtime
tab.vars <- dfh %>%
...
###################################################################
####### Circ Depth / Introduced gates plot
###################################################################

# Create table for QiskitDepth and NumberOfIntroducedGates (^= EstimatedCircDepth)
# Filter the reduced variants
tab.reduced <- dfh %>%
...
# Filter columns before the reduction (will have 3 duplicates)
tab.before <- dfh %>%
...
# Create joint plot for the number of introduced gates and the circuit depth
...
# write.csv(tab.joint.circdepth.intrGates, "tab.joint.circdepth.intrGates.csv")
...
# Create table for the density facet grid plot
# Filter the reduced variants
tab.reduced <- dfh %>%
...
# Collapse Densities into Key Value pair
...
# No need to use TeX("TeX string $4$") here, since we use tikzDevice outputs. Note that the results through ggsave() show non-compiled LaTeX source
...
# Since we are using percent signs, the sanitize option needs to be enabled
