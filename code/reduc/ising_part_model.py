...
class JSS_model_part:
    ...
    ##########################################################################################
    # part | min makespan | Create the Ising model
    ##########################################################################################
    ...
    def create_ising_model_part1(self):
        ...
        mach_pairs = list(itertools.combinations(range(self.num_machines), 2)) #only (0,1)
        job_pairs = list(itertools.combinations(range(self.num_jobs), 2)) #only (0,1)
        ...
        # duration objective: self.durations + initial self.rigs
        H_obj=0
        ...
        variables=[] #here, we have blocks of jobs, not machines
        for i in range(self.num_jobs):
            for j in range(self.num_machines):
        ...
        for var_dict in [linear, quadratic]:
            for key in var_dict.keys():
                # extract indices of all binary variables in the key as list
                nums=[int(s) for s in re.findall(r'\d+', str(key))]
                new_key=[]
                for var_count in range(int(len(nums)/2)):
                    # use that x^2=x for binary variables, and thus also x^3=x, etc.
                ...
                if len(new_key) == 2:
                    ...
                    for perm in list(itertools.permutations(tuple(new_key))):
                        # if another permutation of these indices already existsas key, we add the current value to its value (to account for commutativity)
    ...
    ##########################################################################################
    # create dict from ising model to be fed into quark code by DLR
    ##########################################################################################
    ...
    def create_quark_dict(self):
        ...
        for var_dict in [self.linear, self.quadratic]:
            for key in var_dict.keys():
                ...
                nums=[int(s) for s in re.findall(r'\d+', str(key))]
                new_key=[]
                # divide list in pairs, create binary variables and corresponding indices
                for var_count in range(int(len(nums)/2)):
    ...
    ##########################################################################################
    # part1 | min makespan | Compute qubo penalty and total cost for a solution string
    ##########################################################################################
    def qubo_energy_penalty_min_ms(self, x_val):
        ...
        # duration objective: durations + initial rigs
        energy_obj=0
        for pair in mach_pairs:
            temp1 = sum([x[i,pair[0]] * (self.durations[i]+self.rigs_initial[i,pair[0]]) for i in range(self.num_jobs)]) + sum([x[job_pair[0],pair[0]]*x[job_pair[1],pair[0]]*self.rigs[job_pair[0],job_pair[1]] for job_pair in job_pairs])
            temp2 = sum([x[i,pair[1]] * (self.durations[i]+self.rigs_initial[i,pair[1]]) for i in range(self.num_jobs)]) + sum([x[job_pair[0],pair[1]]*x[job_pair[1],pair[1]]*self.rigs[job_pair[0],job_pair[1]] for job_pair in job_pairs])
    ...
    ###################################################################################################
    # part | Convert bitstring to schedule and vice versa
    ###################################################################################################
   
   
    def dict_to_schedule(self, dict):
        # convert dictionary being produced by the transform_dict_arrays to schedule
    ...
    def schedule_to_bitstring(self, schedule):
        # get bitstring from schedule, useful for testing pruposes to translate a known, optimal solution to a bitstring
    ...
    def opt_dict_to_bitstring_schedule(self, opt_sols):
        ...
        """
        # opt_schedules might containt the same schedule several times. There might be GS, which differ only by switching jobs on a machine. 
        opt_schedules=[]
        opt_bitstrings=[]
