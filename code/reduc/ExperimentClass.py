...
import csv
import json
#from networkx.readwrite import json_graph #dump nx graph
...
class ExperimentClass:
 ...
 "NoTermsDegr1", "NoTermsDegr2", "NoTermsDegr3", "NoTermsDegr4", "EstimatedCircDepthr", "QiskitDepthr", "QiskitDepthTranspiledr", "avgNodeDegreer", "num_edgesr", "num_multi_edgesr"]
        ...
        for nM in range(1,MaxNumMachines):
            for nJ in range(nM, MaxNumJobs):
                for diffi in list(["easy", "medium", "hard"]):
                    #use insert to create an implicit scheduling where long tasks run at the beginning, so that all processes are utilized to the end and the total execution time will decrease
                    #Although map returns the results in parameter order, it might not execute in parameter order
                    argsM.insert(0, nM)
                    argsJ.insert(0, nJ)
    ...
    def executeSingle(self, machines, jobs, difficulty, useQiskitTranspile):
        ...
        #Assumption: single RZ, RZZ: +3, RZZZ: +5, RZZZZ: +7
        ...
        #compact not necessary since we only use single indices already
        #var_pair_choice e {"stupid_but_supposedly_fastest", "probably_better", "supposedly_best_but_slowest"}
        ...
        with open(pathExtended + "polyBefore.json", "w") as pB:
            json.dump(polyBinary.__repr__(), pB) # Dump polyonimal (=dict) into json file
            
        with open(pathExtended + "polyAfter.json", "w") as pA:
            json.dump(CombinedPoly.__repr__(), pA) # Dump polyonimal (=dict) into json file
        
        #Write deg per variable to file
        with open(pathExtended + "GraphDegBefore.json", "w") as gdB:
            json.dump(str(Gdeg), gdB) # Dump (var, deg) pairs into json file
            
        with open(pathExtended + "GraphDegAfter.json", "w") as gdA:
            json.dump(str(Gdegr), gdA) # Dump (var, deg) pairs into json file
        
        #Write full graph to file
        with open(pathExtended + "GraphBefore.json", "w") as gB:
            json.dump(nx.node_link_data(G), gB) # Dump coupling graph into json file
            
        with open(pathExtended + "GraphAfter.json", "w") as gA:
            json.dump(nx.node_link_data(Gr), gA) # Dump coupling graph into json file
        
        #To get the graph back from JSON file:
        """
    ...
    def genExample(num_jobs, num_machines, difficulty):
        ...
        #Create rigs matrix (describes the time needed to switch from one rig to another). It is symmetric.
        #Always create as many rig classes as there are jobs. The difficulty comes into play when assigning them to jobs/ machines
        rigsMatrix = np.zeros(shape=(num_jobs,num_jobs), dtype=np.int32)
        
        #List of Dictionary(duration, rig class)
        jobs = []
        
        #List of dictionary(rig class)
        machines = []
        ...
        if (difficulty == "easy"):
            ...
            for i in range(0,num_jobs):
                rigsMatrix[i][i] = 0 # diagonal elements
        ...
        #Transform matrices to the next integer and scale appropriately
    ...
    #For example: PolyPart(<polyBinary>, 3) will return all monomials with degree 3 from <polyBinary>
    #See Quark Framework for quadratic and linear parts
    @staticmethod
    ...
    #Transforms the penalty dictionary of the form {r_x0x1: {<polydict>}} into a poly dict. Correspondence to the reduced term will be lost
    @staticmethod
    def PenaltyDictToPolyDict(penaltyDict):
        out = dict()
        for key in penaltyDict:
            #Add key, value pair (^= <polydict>)
            t = dict(penaltyDict[key])
            out.update(t)
        return out
    
    #Constructs a QAOA Circuit part (Problem Hamiltonian) from a given polynomial and returns its depth 
    @staticmethod
    def QiskitDepthEstimate(polynomial):
        circ = QuantumCircuit(len(polynomial.variables))
        pC = polynomial.compact()
        for key in pC: #key ^= Term
            deg = len(key)
            if deg == 1:
    ...
    #Generate noise model. Input: 1 qubit gates and 2 qubit gates in gate set
    @staticmethod
    ...
    #Transpile the circuit with max. optimisation level
    @staticmethod
    ...
    def genCouplingGraphFromPolynomial(polynomial):
        ...
        #undirected MultiGraph, since the pair (x1,x2) in x1x2 + x1x2x3 shall be counted twice 
        G = nx.MultiGraph()
        #Every variable is a node
        G.add_nodes_from(pC.variables)
        
        #Get every possible pair from each monomial
        edges = []
